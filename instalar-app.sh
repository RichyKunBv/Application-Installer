...
function actualizar_script() {
    ...
    echo -e "\n${C_YELLOW}› Verificando actualizaciones...${C_RESET}"; local repos_posibles=("Application-Installer-in-ZIP"); local scripts_posibles=("instalar-app.sh"); local url_version_encontrada=""; local url_script_encontrado=""; local exito=false; local download_tool=""; if command -v curl &> /dev/null; then download_tool="curl -sfo"; elif command -v wget &> /dev/null; then download_tool="wget -qO"; else error "Se necesita 'curl' o 'wget'."; fi; for repo in "${repos_posibles[@]}"; do local url_temp_version="https://raw.githubusercontent.com/RichyKunBv/${repo}/main/version.txt"; if curl --output /dev/null --silent --head --fail "$url_temp_version"; then url_version_encontrada="$url_temp_version"; for script_name in "${scripts_posibles[@]}"; do local url_temp_script="https://raw.githubusercontent.com/RichyKunBv/${repo}/main/${script_name}"; if curl --output /dev/null --silent --head --fail "$url_temp_script"; then url_script_encontrado="$url_temp_script"; exito=true; break; fi; done; fi; [ "$exito" = true ] && break; done; if [ "$exito" = false ]; then error "No se pudo encontrar un repositorio o script válido en GitHub."; fi; local version_remota; version_remota=$($download_tool - "$url_version_encontrada"); if [ -z "$version_remota" ]; then error "No se pudo obtener la versión remota."; fi; if dpkg --compare-versions "$version_remota" gt "$VERSION_LOCAL"; then echo -e "${C_GREEN}¡Nueva versión ($version_remota) encontrada!${C_RESET}"; echo -e "${C_YELLOW}Descargando...${C_RESET}"; local script_actual="$0"; local script_nuevo="${script_actual}.new"; if $download_tool "$script_nuevo" "$url_script_encontrado"; then chmod +x "$script_nuevo"; mv "$script_nuevo" "$script_actual"; success "¡Actualizado!"; echo -e "${C_YELLOW}Vuelve a ejecutar el script.${C_RESET}"; exit 0; else error "Error al descargar."; rm -f "$script_nuevo"; fi; else success "Ya tienes la última versión ($VERSION_LOCAL)."; fi
}
